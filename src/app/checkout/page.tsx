...
import { getFreightEstimate } from './actions'
...
export default function CheckoutPage() {
  ...
  const getProfile = async () => {
    ...
    // Get addresses manually because we are in a client component
    ...
    if (profile?.addresses) {
      ...
      else if (profile.addresses.length > 0 && !selectedAddressId) {
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {addresses.map((addr) => (
                      <div 
                        ...
                        className={`relative p-8 rounded-[40px] border-4 cursor-pointer transition-all ${
                          selectedAddressId === addr.id 
                          ? 'border-secondary bg-white shadow-2xl ring-4 ring-secondary/5' 
                      ...
                      >
                        ...
                        <p className="font-black text-primary mb-1">{addr.fullName}</p>
                        <p className="text-sm text-muted-foreground font-medium leading-relaxed">
                          {addr.street}, {addr.city}<br/>
                          {addr.country}
                        </p>
                        <p className="mt-4 text-xs font-black text-primary underline decoration-secondary decoration-2 underline-offset-4">
                          {addr.phone}
                        </p>
                      </div>
                    ))}
                    
                     {/* Reuse AddressManager for new address modal - customized to only show the button */}
                     <div className="col-span-full pt-4">
                        <AddressManager addresses={[]} onSuccess={getProfile} />
                     </div>
                  </div>
                )}
              </section>
