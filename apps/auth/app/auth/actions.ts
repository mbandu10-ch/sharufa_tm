...
import { createAdminClient } from '@root/utils/supabase/admin'
import { prisma } from '@root/lib/prisma'
import { sendEmail, ConfirmEmail, ResetPasswordEmail } from '@sharufa/emails'
...
export async function login(formData: FormData) {
  ...
  if (next && next !== '/dashboard' && next !== '/seller' && next !== '/admin' && next !== '/cargo') {
    ...
    if (isPublicContent) {
      return redirect(getSafeReturnTo(next))
    }
  }
